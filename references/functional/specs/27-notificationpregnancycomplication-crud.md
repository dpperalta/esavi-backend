...
- [ ] **El escenario de colisión:** crear A y B, desactivar **B** —la de mayor `sortOrder`—, crear C —que toma el número liberado—, reactivar B responde **200**, B reaparece con un `sortOrder` nuevo al final, y **no** se produce error de índice único.
- [ ] **La variante que no colisiona:** desactivar **A** en vez de B deja un hueco que el trigger nunca rellena —C nace con 3, no con 1—, así que reactivar A responde **200** y conserva su número. El trigger cuenta `COALESCE(MAX("sortOrder"), 0) + 1` sobre las filas vivas, y el número solo se reutiliza cuando la fila retirada era la última.
