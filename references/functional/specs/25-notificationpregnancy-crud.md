...
> **Estado:** Aprobado
> **Depende de:** SPEC 01 (roles), SPEC 02 (validación de entrada), SPEC 03 (paridad i18n), SPEC 05 (códigos de operación), SPEC 08 (`lang` requerido en servicios), **SPEC F10 (`notification` — dependencia dura de modelo: es el padre de la FK y la fuente de la visibilidad heredada)**, **SPEC F26 (`systemConfig` — dependencia dura de implementación, ya redactado e implementado: la regla del sexo femenino lee de ahí el `catalogItemId`)**, SPEC F13 (`severeNotification` — precedente de la satélite uno a uno, y quien dejó abierta la pregunta que §6 de este spec responde), SPEC F24 (`notificationDiluent` — precedente inmediato de la familia y del `005C`), SPEC F08 (operación `005C` de borrado físico), SPEC F12 (`buildDifferentialUpdate` — el `004` lo usa)
> **Fecha:** 2026-08-18 · **Revisado:** 2026-08-19, tras la implementación de [F26](./26-systemconfig-crud.md). Los cambios de esa revisión están acotados a la regla del sexo femenino —contrato exacto de la fila de configuración, cómo se lee y las cinco causas del 500—, a la decisión de no sembrarla por el `008`, y al recuento de `ROUTE_RULES`, que pasa de 187 a 194
...
**D — Es el primer consumidor de `systemConfig` del repositorio.** La regla del sexo lee el `catalogItemId` femenino de `systemConfig` (`:358-377`), que [F26](./26-systemconfig-crud.md) ya entregó con modelo, servicio y ruta. Cuando F25 se redactó, esa entidad no existía en `src/` y el bloqueo era duro; hoy la dependencia está satisfecha y lo que queda es una **precondición de datos**: la fila `PREGNANCY_FEMALE_SEX_ITEM` tiene que existir en cada despliegue. Es también la primera vez que un servicio de este repositorio lee configuración de despliegue en lugar de una constante del código, y §6 razona por qué aquí no cabía la constante.

F26 dejó escrito en su `src/data/systemConfig.defaults.ts` que **nadie consume todavía** esas filas desde el código de arranque. Sigue siendo cierto de los ocho parámetros que aquel catálogo siembra: F25 no consume ninguno de ellos, sino una clave de dominio propia que no cabe en ese catálogo por la razón que §2 declara.
...
- **Regla del sexo femenino, solo en el `001`.** El `catalogItemId` del sexo femenino se lee de `systemConfig` con `code: 'PREGNANCY_FEMALE_SEX_ITEM'` y `scope: 'GLOBAL'`. Si el paciente del caso tiene `sexItemId` con valor y **no** coincide → 400 `NOTIFPRG_001_PATIENT_NOT_FEMALE`. Cinco precisiones que son parte de la regla:
  - **`sexItemId` en `null` no bloquea.** Sexo desconocido no es prueba de que no haya embarazo.
  - **La fila se lee con `SystemConfig.findOne` desde este servicio**, nunca a través de `getSystemConfigByCodeService` (el `006` de F26). Ese servicio enmascara el `value` para todo lo que no sea SUPERADMIN y lanza su propio 404, y el `001` de F25 corre en rol **USER**: entrar por ahí devolvería `null` o un error ajeno con el código de otra entidad.
  - **El contrato exacto de la fila es `valueType: 'string'`, `scope: 'GLOBAL'`, `isEncrypted: false`**, y su `value` es el UUID del `catalogItem` femenino como cadena JSON. La columna es `jsonb`, no `varchar`: lo que se compara contra `sexItemId` es el contenido de ese `value`, no la fila.
  - **Cualquier desviación de ese contrato es 500**, no 400: fila ausente, inactiva, con `valueType` distinto de `'string'`, con un `value` que no es UUID, con `isEncrypted: true`, o apuntando a un `catalogItemId` inexistente. Es un fallo de configuración del despliegue y el notificador no puede arreglarlo. **El caso de `isEncrypted: true` es el que hay que atrapar explícitamente:** F26 guarda ahí `{ "enc": "<ciphertext>" }`, que jamás coincidiría con un `sexItemId`, y sin esta guarda toda paciente femenina recibiría un `400 PATIENT_NOT_FEMALE` que culpa al notificador de un error del despliegue.
...
- Siete filas nuevas en `ROUTE_RULES` de `tests/auth/roles.test.ts` —de **187 a 194**— y suite `tests/contract/notificationPregnancy.test.ts`.

**Precondiciones de datos** (no son parte de la implementación):

- **`systemConfig` ya está implementado**, por [F26](./26-systemconfig-crud.md). La dependencia de código que este spec declaraba como bloqueo está satisfecha, y lo que queda es la precondición de datos de los dos puntos siguientes.
- **La clave `PREGNANCY_FEMALE_SEX_ITEM` debe existir y apuntar a un `catalogItem` real**, con `valueType: 'string'`, `scope: 'GLOBAL'` e `isEncrypted: false`. Sin ella el `001` responde 500 en cada intento. Se crea por el **`ESAVI-SYSCONF-001`** en cada despliegue, no por código que este spec escriba.
- **La clave NO va en `src/data/systemConfig.defaults.ts`, y esto es una decisión, no un olvido.** El catálogo del `008` es un fichero de código con valores fijos, y el `value` de esta clave es un `catalogItemId` que **cambia en cada instalación**. Sembrarla desde ahí plantaría en producción un UUID de desarrollo que apunta a un ítem inexistente, y el `008` es solo-alta: la fila mala quedaría fija y ningún `POST /sync` posterior la corregiría. La razón está en §6.
...
- **El CRUD de `systemConfig`.** Este spec **lee** una clave y no escribe ninguna. Todo lo demás de esa entidad —`valueType` con su `CHECK`, `scope` en la `UNIQUE` compuesta, `isEncrypted`, `isEditable`, la tabla `systemConfigHistory` y la siembra del `008`— lo entregó [F26](./26-systemconfig-crud.md).
...
Siete filas nuevas en `ROUTE_RULES`: de **187** a **194**. El punto de partida lo dejó F26 en `tests/auth/roles.test.ts:395`, que subió el total de 177 a 187 con las diez rutas de `systemConfig`.
...
3. **Regla del sexo femenino.** Se lee con `SystemConfig.findOne({ where: { code, scope, isActive: true } })` **desde este servicio**, sobre el modelo que F26 dio de alta y **nunca** a través de `getSystemConfigByCodeService` — ese servicio enmascara el `value` para todo lo que no sea SUPERADMIN, y el `001` corre en rol USER. `code` y `scope` van desde las constantes ya en constant-case, que es la forma en que F26 los almacena tras su `toConstantCase`, así que no hace falta normalizar nada aquí.

   La fila esperada es `code: 'PREGNANCY_FEMALE_SEX_ITEM'`, `scope: 'GLOBAL'`, `valueType: 'string'`, `isEncrypted: false`, y su `value` —columna `jsonb`— es el `catalogItemId` femenino como cadena JSON. La comparación se hace contra ese `value`, no contra la fila.
   - **Cinco causas de 500 `NOTIFPRG_001_SEX_CONFIG_MISSING`**, y son una sola comprobación con cinco salidas: la fila **no existe**, está **inactiva**, tiene `valueType` **distinto de `'string'`**, tiene **`isEncrypted: true`**, o su `value` **no es un UUID de un `catalogItem` existente**. Es un fallo de configuración del despliegue, no del cliente.
   - **El caso de `isEncrypted: true` se comprueba explícitamente y no se resuelve descifrando.** F26 guarda ahí `{ "enc": "<ciphertext>" }`: sin la guarda, el objeto envuelto nunca coincidiría con un `sexItemId` y toda paciente femenina recibiría un **400 `PATIENT_NOT_FEMALE`** —un error de despliegue disfrazado de error del notificador, que es exactamente lo que §6 decidió evitar al mandar estos fallos al 500—. Descifrarlo con `decryptSystemConfigValue` tampoco es la salida: un `catalogItemId` no es un secreto, y aceptar la fila cifrada normalizaría una configuración mal declarada.
...
| `notificationPregnancy.sexConfigMissing` | 500 cuando la clave `PREGNANCY_FEMALE_SEX_ITEM` falta, está inactiva, tiene un `valueType` distinto de `'string'`, está marcada `isEncrypted: true`, o apunta a un `catalogItem` inexistente. **Una sola clave para las cinco causas**: al notificador no le sirve distinguirlas, y quien las diagnostica lee el log |
...
> **El bloqueo previo está levantado.** Este plan se escribió cuando `systemConfig` no tenía modelo y el paso 8 no podía ejecutarse. [F26](./26-systemconfig-crud.md) lo entregó, así que los diecisiete pasos son ejecutables en orden y el 8 ya no es condicional. Lo que sobrevive es la **precondición de datos**: la fila `PREGNANCY_FEMALE_SEX_ITEM` tiene que estar cargada, con el contrato de §3.5, para que la verificación del paso 8 sea posible.
...
8. **La regla del sexo femenino.** El paso 3 de §3.5, dentro del mismo `001`: lectura con `SystemConfig.findOne` por `code` y `scope` con `isActive: true`, las cinco guardas de contrato que caen en el 500, y comparación contra el `patient.sexItemId` que el `include` de tres saltos ya trajo. La fila de prueba se crea desde la suite por el `ESAVI-SYSCONF-001`, no desde `systemConfig.defaults.ts`.
   *Verificación:* con la clave sembrada, crear sobre un caso cuyo paciente es masculino devuelve **400** `NOTIFPRG_001_PATIENT_NOT_FEMALE`; sobre un paciente femenino devuelve 201; sobre un paciente con `sexItemId: null` devuelve **201**; borrando la fila de `systemConfig`, cualquier alta devuelve **500** `NOTIFPRG_001_SEX_CONFIG_MISSING` y no 400; desactivándola, lo mismo; apuntándola a un `catalogItemId` inexistente, lo mismo; **con `valueType: 'number'`, lo mismo**; **creándola con `isEncrypted: true`, lo mismo — y nunca 400**, que es la verificación que atrapa el fallo silencioso; el servicio **no** llama a `getSystemConfigByCodeService`: `grep -n "getSystemConfigByCode" src/services/notificationPregnancy.service.ts` no devuelve resultados; la regla **no cuesta una segunda consulta al paciente** — el servicio no contiene ninguna llamada a `Patient.findOne`.
...
16. **Cubrir las siete rutas en `tests/auth/roles.test.ts`.** Siete filas en `ROUTE_RULES` con su `minRole` y su código, y subir el total esperado de **187 a 194** en la aserción de longitud (`tests/auth/roles.test.ts:395`, que F26 dejó en 187).
...
- [ ] `ROUTE_RULES` tiene 194 filas y la aserción de longitud de `tests/auth/roles.test.ts` espera ese número.
...
- [ ] Con la fila de `systemConfig` ausente, inactiva, con `valueType` distinto de `'string'`, con **`isEncrypted: true`**, o apuntando a un `catalogItemId` inexistente, el `001` responde **500** `NOTIFPRG_001_SEX_CONFIG_MISSING` — nunca 400, y nunca 201. Son **cinco** escenarios en la suite.
- [ ] El escenario de `isEncrypted: true` responde 500 y **no** `400 PATIENT_NOT_FEMALE` sobre una paciente femenina. Es el fallo silencioso que F26 introdujo al envolver el valor cifrado en `{ "enc": ... }`, y el único de los cinco que produciría una respuesta plausible pero falsa.
- [ ] El servicio lee la configuración con `SystemConfig.findOne` y no por el `006` de F26: `grep -n "getSystemConfigByCode" src/services/notificationPregnancy.service.ts` no devuelve resultados.
- [ ] `PREGNANCY_FEMALE_SEX_ITEM` **no** aparece en `src/data/systemConfig.defaults.ts`: `grep -n "PREGNANCY_FEMALE_SEX_ITEM" src/data/systemConfig.defaults.ts` no devuelve resultados. Su `value` es un UUID que cambia por instalación y el `008` es solo-alta.
...
- **Sí:** leer el `catalogItemId` femenino de `systemConfig`, aunque cuando este spec se redactó eso convirtiera a una entidad sin implementar en dependencia dura. F26 la entregó y la dependencia está satisfecha. Las alternativas eran peores, y las tres se consideraron:
...
- **Sí:** extender ese 500 a **`valueType` distinto de `'string'`** y a **`isEncrypted: true`**, que son dos causas que este spec no contemplaba porque `systemConfig` no existía cuando se escribió. El `value` de F26 es `jsonb` gobernado por `valueType`, y su `isEncrypted: true` guarda `{ "enc": "<ciphertext>" }`: una fila así **no falla**, simplemente nunca coincide con ningún `sexItemId`, y sin la guarda el resultado sería un `400 PATIENT_NOT_FEMALE` sobre pacientes que sí lo son. Es el único fallo de configuración de este spec que produciría una respuesta plausible en vez de un error, y por eso se atrapa explícitamente en vez de dejarlo caer por comparación.
- **No:** descifrar la fila con `decryptSystemConfigValue` cuando llega marcada `isEncrypted: true`, en lugar de rechazarla. Funcionaría, y precisamente por eso es peor: normalizaría una configuración mal declarada y dejaría un `catalogItemId` —que no es un secreto— pagando cifrado y descifrado en cada alta, con el riesgo añadido de que el día que `esaviCrypt` cambie de clave la regla del sexo empiece a rechazar pacientes femeninas.
- **Sí:** leer con `SystemConfig.findOne` directo y no por `getSystemConfigByCodeService`. Reutilizar el servicio de F26 parece lo correcto y no lo es: enmascara el `value` para todo lo que no sea SUPERADMIN —y el `001` corre en USER—, y lanza un 404 con el código de otra entidad, que se propagaría al notificador como si el embarazo no existiera. Un servicio que existe para una lectura HTTP no es la vía para una lectura interna.
- **Sí:** que `PREGNANCY_FEMALE_SEX_ITEM` **no** entre en `src/data/systemConfig.defaults.ts`. El catálogo del `008` es un fichero de código con valores fijos, pensado para parámetros cuyo valor por defecto es el mismo en todas las instalaciones —los ocho que F26 sembró lo son—. El `value` de esta clave es un `catalogItemId` que **cambia en cada base**, así que sembrarla desde ahí plantaría en producción un UUID de desarrollo apuntando a un ítem inexistente. Y como el `008` es **solo-alta**, esa fila mala quedaría fija: ningún `POST /sync` posterior la corregiría, y el síntoma sería un 500 permanente que parece un fallo de siembra cuando en realidad es una siembra que sí ocurrió, con el dato equivocado. Se crea por el `ESAVI-SYSCONF-001` en cada despliegue, donde el UUID se toma del catálogo de sexo de esa instalación.
...
| Riesgo | Mitigación |
|---|---|
| **La fila de configuración llega marcada `isEncrypted: true`** y el `001` responde `400 PATIENT_NOT_FEMALE` sobre pacientes que sí lo son: el despliegue parece funcionar y rechaza todas las altas legítimas con un mensaje que culpa al notificador | Es el riesgo **más peligroso del spec**, porque es el único fallo de configuración que produce una respuesta plausible en vez de un error. Nace de F26, que envuelve el valor cifrado en `{ "enc": "<ciphertext>" }`. §3.5 lo atrapa como una de las cinco causas del 500, §6 razona por qué no se resuelve descifrando, y §5 tiene un criterio dedicado que exige 500 y **nunca** 400 |
| La fila existe pero con **`valueType` distinto de `'string'`**, y el `value` `jsonb` no es la cadena que la comparación espera | Quinta causa del 500 en §3.5, con su criterio en §5. El contrato completo de la fila —`valueType`, `scope`, `isEncrypted`— está declarado en §2 y en §3.5, no se deduce |
| **`PREGNANCY_FEMALE_SEX_ITEM` se añade a `src/data/systemConfig.defaults.ts`** «para que el sync la siembre», y producción acaba con un UUID de desarrollo apuntando a un `catalogItem` inexistente | El `008` de F26 es **solo-alta**: la fila mala quedaría fija y ningún `POST /sync` posterior la corregiría, con un 500 permanente que parece falta de siembra cuando la siembra sí ocurrió. §2 lo declara como decisión explícita, §6 lo razona, y §5 lo vigila por ausencia con un `grep` sobre el fichero de defaults |
| El servicio lee la configuración por **`getSystemConfigByCodeService`** en vez de por el modelo, y el enmascarado de F26 le devuelve `null` a un rol USER — o el 404 de aquella entidad se propaga como si el embarazo no existiera | §3.5 y §6 fijan la lectura con `SystemConfig.findOne`, y §5 lo verifica por ausencia con `grep -n "getSystemConfigByCode"` sobre el servicio |
| **La fila `PREGNANCY_FEMALE_SEX_ITEM` no está sembrada el día del despliegue** y el `001` responde 500 en cada intento: el endpoint nace inservible | Declarado en §2 como precondición de datos. Es el riesgo que **no se cierra con código ni con pruebas**: la fila se carga por el `ESAVI-SYSCONF-001` antes de abrir el endpoint, y conviene verificarlo en el mismo despliegue. El 500 y su clave i18n propia hacen que el fallo se diagnostique en un vistazo del log |
| **El 500 de configuración se «arregla» convirtiéndolo en 400**, y el notificador pasa a buscar el error en su formulario | §6 razona por qué es 500, §3.5 lo fija y §5 lo verifica con cinco escenarios: clave ausente, inactiva, con `valueType` incorrecto, con `isEncrypted: true`, y apuntando a un `catalogItemId` inexistente. Los cinco responden 500, nunca 400 y nunca 201 |
...
- **El CRUD de `systemConfig`.** F25 lee una clave y no escribe ninguna. `valueType`, `scope`, `isEncrypted`, `isEditable`, `systemConfigHistory` y la siembra del `008` los entregó F26, que se implementa **antes** que éste.
- **Sembrar la fila `PREGNANCY_FEMALE_SEX_ITEM`**, ni el catálogo de sexo al que apunta. Son precondiciones de datos, no código que este spec escriba. En particular, **la clave no se añade a `src/data/systemConfig.defaults.ts`**: su `value` es un UUID que cambia por instalación y el `008` es solo-alta.
- **Cambiar el régimen de cifrado de la clave de configuración.** F25 rechaza con 500 una fila marcada `isEncrypted: true` y no la descifra ni la corrige; arreglarla es una escritura por el `004` de F26, con su historial.
