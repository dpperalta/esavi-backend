...
| ~~Cambiar la contraseña **no invalida el token en curso**: quien haya robado un token sigue operando hasta que expire, aunque la víctima cambie la contraseña~~ **RESUELTO por el [SPEC F42](./42-auth-refresh-token.md)** | `ESAVI-USER-006` revoca ahora todas las sesiones del usuario con `revokedReason: 'PASSWORD_CHANGED'`, en la misma transacción que el `UPDATE` de la contraseña. El refresh token robado muere en el acto; el access token en vuelo sobrevive como mucho `JWT_EXPIRES_IN`, que el mismo spec bajó de `8h` a `1h`, y ya no puede renovarse |
