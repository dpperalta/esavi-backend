...
> ---
>
> **Enmienda del 2026-08-16 — el `code` se acuña del `name`.** Posterior a la implementación de este spec y aplicada sobre ella. El `code` de un `catalogItem` **ya no se escribe ni se envía**: se acuña de su `name` en camelCase con `toCodeFromName` (`toTitleCase` y después `toCamelCase`, para que la caja de la entrada no acuñe dos códigos del mismo nombre), en el `001`, en el `004` y en el `006` por igual. `Pharmaceutical form` acuña `pharmaceuticalForm`. Consecuencias, todas normativas y ya implementadas:
>
> - El fichero pasa a **seis cabeceras** y `code` deja de ser una de ellas: si viene, se anota en `unknownHeaders` y se ignora. Las obligatorias son `catalogTypeCode`, `catalogTypeName` y `name`.
> - **El `name` es la identidad** de un ítem dentro de su tipo. Dos filas con el mismo `name` bajo el mismo tipo son la misma fila (`DUPLICATE_IN_FILE`), y un `name` cambiado en el fichero **no renombra: inserta**. Renombrar sigue siendo trabajo del `004`.
> - `EMPTY_CODE` ya no nombra una celda: es un `name` que no acuña nada —`---`—. Un `name` legal cuyo código no cabe en `varchar(100)` sale `VALUE_TOO_LONG` con `column: 'code'`.
...
Ese cruce de tablas obliga además a una precisión que no se ve a simple vista: **cada tabla acuña su `code` de una celda distinta**. `catalogType.code` sale de la celda `catalogTypeCode` con `toCamelCase`, y `catalogItem.code` sale de su propio `name` con `toCodeFromName`. Las dos reglas dan camelCase pero no leen lo mismo, y el importador aplica la de cada una.
...
- **Seis cabeceras**, con emparejamiento normalizado —minúsculas, sin espacios, guiones ni guiones bajos—: `catalogTypeCode`, `catalogTypeName`, `name`, `value`, `description`, `sortOrder`. **No hay `code`**: se acuña del `name`, y una columna `code` en el fichero entra en `unknownHeaders` y se ignora.
- Cabeceras **obligatorias**: `catalogTypeCode`, `catalogTypeName`, `name`. Si falta alguna, la operación corta con 400 antes de leer una sola fila de datos.
...
- **Normalización al escribir**, igual que el `001` y a diferencia de F17 y F19: `toTitleCase` en `catalogItem.name` y en `catalogType.name`, **`toCamelCase`** en `catalogType.code`, y `catalogItem.code` **acuñado del `name`** con `toCodeFromName`. La unicidad se compara siempre contra el valor ya normalizado.
...
| `code` | `varchar(100) NOT NULL` | **no viene del fichero**: acuñado del `name` con `toCodeFromName` |
...
export interface ParsedCatalogItemRow {
    ...
    code: string;                 // acuñado del name ya normalizado, con toCodeFromName
...
8. `catalogTypeCode` vacío → `EMPTY_CATALOG_TYPE_CODE`. `name` vacío → `EMPTY_NAME`, que además rompería el `NOT NULL` de su columna. Un `name` que **no acuña código** —solo separadores, `---`— → `EMPTY_CODE`: pasa el `NOT NULL` y dejaría dos filas ocupando el mismo par vacío.
...
11. Un texto que excede el límite de su columna → rechazo `VALUE_TOO_LONG` con el nombre de la columna. Sin este filtro la fila reventaría el lote. El `code` se comprueba **el último**: un `name` de más de 250 desborda los dos y hay que señalar la celda que el operador escribió, así que nombrar `code` queda para el caso que sí es suyo — un `name` dentro de sus 250 cuyo código no cabe en sus 100.
12. Par `(catalogTypeCode, code)` repetido **dentro del propio fichero**: **gana la primera aparición**; las siguientes se rechazan como `DUPLICATE_IN_FILE`. El duplicado es del **par**, no del `code` solo: `Activo` en dos tipos distintos son dos ítems legítimos. Como el `code` se acuña del `name`, el par se compara de hecho sobre `(catalogTypeCode, name)`: `activo` y `ACTIVO` bajo el mismo tipo son la misma fila.
...
| `code` | **no entra** | la otra mitad; la fila se encontró *por* el par. Se acuña del `name`, así que un `name` cambiado **no es un renombrado sino una fila nueva** |
...
2. **Tipos y parser.** Las cuatro interfaces de §3.3 en `src/types/catalog/catalogItem.types.ts`. `src/helpers/catalogItemParser.helper.ts` con `parseCatalogItemsXlsxFile(buffer)`, el mapa de las seis cabeceras, los límites por columna y los doce puntos de la fase 2, con su alta en `src/helpers/index.ts`. **Función pura, sin acceso a base de datos.**
   *Verificación:* prueba unitaria sobre libros construidos en memoria — `ACTIVO` bajo dos `catalogTypeCode` distintos salen como **dos filas válidas** y no como duplicado; el mismo par repetido produce un `DUPLICATE_IN_FILE`; `Pharmaceutical form` acuña `pharmaceuticalForm` y `pharmaceutical FORM` acuña el mismo; una hoja sin `name` produce el corte por cabecera obligatoria; una hoja **con** `code` lo anota en `unknownHeaders` y lo ignora; una celda vacía sale `null` y no `''`; `sortOrder` vacío, `-1`, `abc` y `40000` salen todos `0` con `sortOrderCoerced: true`, y `0` explícito sale `0` con `sortOrderCoerced: false`; un `catalogTypeName` vacío **no** rechaza la fila.
...
7. **Fixture y suite de contrato.** `tests/fixtures/catalog-items-sample.xlsx` con la cabecera de seis columnas y ocho filas de datos: **cuatro** ítems de un tipo **existente** —la cuarta con el `sortOrder` inválido—, dos de un tipo **inexistente** con su `catalogTypeName`, uno con el `name` repetido dentro del mismo tipo en otra caja, y uno con un `name` que no acuña código. Bloque nuevo en `tests/contract/catalogItem.test.ts` que sube el fichero con `.attach()`.
   *Verificación:* el informe devuelve `read: 8, inserted: 6, invalid: 1, duplicated: 1, catalogTypesCreated: 1, sortOrderCoerced: 1` — y `8 = 6 + 1 + 1`, que es la comprobación de que los contadores cierran; la reimportación sale `unchanged: 6` con `catalogTypesCreated: 0`; cambiar `value` en una fila del libro produce `updated: 1`, y cambiar su `name` produce `inserted: 1` sin renombrar la existente. **El fixture no puede quedar ignorado por git**: `.gitignore` tiene un `*.xlsx` global, ya excepcionado en `!tests/fixtures/*.xlsx` por F19 — comprobar con `git check-ignore -v` que la excepción sigue en pie.
...
- [ ] Dos ítems con el mismo `name` —y por tanto el mismo `code`— bajo **tipos distintos** se importan los dos. La unicidad es del par, y ése es el criterio que distingue este importador de los dos anteriores.
...
- [ ] `code` se acuña del `name` con `toCodeFromName` y `name` se guarda con `toTitleCase`, igual que por el `001`: importar `  activo simple  ` y crearlo por el `001` producen **la misma fila** —`code: 'activoSimple'`—, y la segunda vía responde 409 `CATITEM_001_CODE_EXISTS`.
- [ ] `catalogTypeCode` se normaliza con **`toCamelCase`** sobre su propia celda, y no se acuña del `catalogTypeName`: importar un fichero cuyo `catalogTypeCode` es `tipo evento` resuelve contra el `catalogType` que el `ESAVI-CATTYPE-001` habría creado con esa misma entrada, y **no** funda uno nuevo.
...
- [ ] Un libro sin la columna `name` devuelve **400** `CATITEM_006_FILE_INVALID` y **no escribe ninguna fila en ninguna de las dos tablas**. Un libro **con** una columna `code` se importa igual, con `code` en `unknownHeaders`.
...
- **Sí:** mantener la normalización de escritura —el `code` acuñado del `name`, `toTitleCase` en el `name`—, al revés que F17 y F19. Aquellos importan dato citado de un diccionario licenciado y por eso solo recortan; un `catalogItem` es configuración acuñada en esta aplicación, y si el importador no normalizara, el mismo valor entraría distinto según la puerta —`001` o `006`— y la unicidad dejaría de detectar el duplicado.
- **Sí:** aplicar **`toCamelCase` a `catalogType.code`** sobre su celda y acuñar `catalogItem.code` de su `name`. El importador respeta lo que hace cada servicio, porque su trabajo es resolver contra lo que ya existe. *(Enmienda: las dos reglas dan hoy camelCase; en la versión original de este spec eran `toCamelCase` frente a `toConstantCase`, y unificarlas se dejó fuera. La unificación llegó después, aplicada a la vez sobre el `001`, el `004` y el `006`, y **sin migración de datos**.)*
...
| Los dos `code` se acuñan de la misma celda y **cada carga funda un tipo duplicado** del que ya existe | El de `catalogType` sale de la celda `catalogTypeCode` y el de `catalogItem` de su `name`; están en la tabla de §3.5 fila por fila, y hay un criterio de aceptación propio para cada uno |
| **Una reimportación de un fichero cargado antes de la enmienda duplica todo el catálogo**, ahora en camelCase junto a los códigos viejos en `CONSTANT_CASE` | Asumido: la enmienda rige de aquí en adelante y no migra datos. `dryRun` lo hace visible antes de escribir — un fichero ya cargado que devuelve `inserted` en vez de `unchanged` es exactamente esta situación |
