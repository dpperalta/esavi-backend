...
7. **Fixture y suite de contrato.** `tests/fixtures/catalog-items-sample.xlsx` con la cabecera de siete columnas y ocho filas de datos: **cuatro** ítems de un tipo **existente** —la cuarta con el `sortOrder` inválido—, dos de un tipo **inexistente** con su `catalogTypeName`, uno con `code` repetido dentro del mismo tipo, y uno con `code` vacío. Bloque nuevo en `tests/contract/catalogItem.test.ts` que sube el fichero con `.attach()`.
