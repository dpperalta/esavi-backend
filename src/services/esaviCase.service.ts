...
// The third satellite of the investigation branch, and the NINTH sibling of this block. It is
// necessary and not redundant, by the exact reason the seventh and the eighth document: the mass
// Investigation.update further up does NOT go through setInvestigationActivationService, so the
// cascade SPEC F32 installed there never fires from here. Without this function the medical history
// of an investigation dragged by its case would stay unsealed — invisible but not sealed, and
// therefore never purgable. It is the easiest mistake to make in that spec, because the previous
// step looks like it already solved it and it does not.
//
// investigationMedicalHistory has no isActive column either, so what moves is its deletedAt, and
// rows already sealed are left alone: they keep their original date and receive no new entry.
// ESAVI-CASE-005B clears nothing, coherent with F07, F29 and F30
const cascadeSealInvestigationMedicalHistories = async (caseId: string, authUser: AuthUser | undefined, transaction: Transaction) => {
    const investigations = await Investigation.findAll({
        where: { caseId },
        attributes: ['investigationId'],
        transaction
    ...
    await cascadeSealSatellite({
        ...
        where: { investigationId: investigations.map(investigation => investigation.investigationId) },
        method: 'ESAVI-CASE-005A',
        detail: 'Investigation medical history sealed by cascade from its ESAVI Case',
        authUser,
        transaction
    });
}
...
// The fifth satellite of the investigation branch, and the ELEVENTH sibling of this block. It is
// necessary and not redundant, by the exact reason the four above document: the mass
// Investigation.update further up does NOT go through setInvestigationActivationService, so the
// cascade SPEC F36 installed there never fires from here. Without this function the vaccination
// context of an investigation dragged by its case would stay unsealed - invisible but not sealed,
// and therefore never purgable.
//
// investigationVaccinationContext has no isActive column either, so what moves is its deletedAt, and
// rows already sealed are left alone: they keep their original date and receive no new entry.
// ESAVI-CASE-005B clears nothing, coherent with F07, F29, F30, F32 and F34
const cascadeSealInvestigationVaccinationContexts = async (caseId: string, authUser: AuthUser | undefined, transaction: Transaction) => {
    const investigations = await Investigation.findAll({
        where: { caseId },
        attributes: ['investigationId'],
        transaction
    ...
    await cascadeSealSatellite({
        ...
        where: { investigationId: investigations.map(investigation => investigation.investigationId) },
        method: 'ESAVI-CASE-005A',
        detail: 'Investigation vaccination context sealed by cascade from its ESAVI Case',
        authUser,
        transaction
    });
}

// The same reasoning for the eighth satellite of investigation. Investigation.update further up
// does NOT go through setInvestigationActivationService, so the cascade SPEC F38 installed there
// never fires from here. Without this function the cold chain of an investigation dragged by its
// case would stay unsealed - invisible but not sealed, and therefore never purgable.
//
// investigationColdChain has no isActive column either, so what moves is its deletedAt, and rows
// already sealed are left alone: they keep their original date and receive no new entry.
// ESAVI-CASE-005B clears nothing, coherent with F07, F29, F30, F32, F34 and F36
const cascadeSealInvestigationColdChains = async (caseId: string, authUser: AuthUser | undefined, transaction: Transaction) => {
    const investigations = await Investigation.findAll({
        where: { caseId },
        attributes: ['investigationId'],
        transaction
    ...
    await cascadeSealSatellite({
        ...
        where: { investigationId: investigations.map(investigation => investigation.investigationId) },
        method: 'ESAVI-CASE-005A',
        detail: 'Investigation cold chain sealed by cascade from its ESAVI Case',
        authUser,
        transaction
    });
}

// The same reasoning for the ninth satellite of investigation. Investigation.update further up does
// NOT go through setInvestigationActivationService, so the cascade SPEC F39 installed there never
// fires from here. Without this function the administration error of an investigation dragged by its
// case would stay unsealed - invisible but not sealed, and therefore never purgable.
//
// investigationAdministrationError has no isActive column either, so what moves is its deletedAt,
// and rows already sealed are left alone: they keep their original date and receive no new entry.
// ESAVI-CASE-005B clears nothing, coherent with F07, F29, F30, F32, F34, F36 and F38
const cascadeSealInvestigationAdministrationErrors = async (caseId: string, authUser: AuthUser | undefined, transaction: Transaction) => {
    const investigations = await Investigation.findAll({
        where: { caseId },
        attributes: ['investigationId'],
        transaction
    ...
    await cascadeSealSatellite({
        ...
        where: { investigationId: investigations.map(investigation => investigation.investigationId) },
        method: 'ESAVI-CASE-005A',
        detail: 'Investigation administration error sealed by cascade from its ESAVI Case',
        authUser,
        transaction
    });
}
