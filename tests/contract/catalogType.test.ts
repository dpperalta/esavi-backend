    ...
    describe('sysDetails alignment — SPEC F52', () => {
        ...
        beforeAll(async () => {
            const created = await request(app)
                .post('/api/catalog-types')
                .set(authHeader('SUPERADMIN'))
                .send({ name: `SysDetails ${ suffix }` });
            expect(created.status).toBe(201);
            id = created.body.data.catalogTypeId;
        });

        it('is absent from 002A (USER, active only)', async () => {
            const response = await request(app).get('/api/catalog-types?limit=100').set(authHeader('USER'));
            expect(response.status).toBe(200);
            const row = response.body.data.rows.find((r: { catalogTypeId: string }) => r.catalogTypeId === id);
        ...
        it('is absent from 002B (SUPERADMIN, including inactive)', async () => {
            const response = await request(app).get('/api/catalog-types?limit=100').set(authHeader('SUPERADMIN'));
            expect(response.status).toBe(200);
            const row = response.body.data.rows.find((r: { catalogTypeId: string }) => r.catalogTypeId === id);
            expect(row).not.toHaveProperty('sysDetails');
            expect(row).toHaveProperty('appDetails');
        ...
        it('is absent from 003 (get by id)', async () => {
            const response = await request(app).get(`/api/catalog-types/${ id }`).set(authHeader('USER'));
            expect(response.status).toBe(200);
