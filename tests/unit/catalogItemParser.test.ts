...
// The contract suite covers the endpoint; this one covers the seven headers and the twelve rules

const FULL_HEADER = [ 'catalogTypeCode', 'catalogTypeName', 'code', 'name', 'value', 'description', 'sortOrder' ];
...
describe('parseCatalogItemsXlsxFile', () => {
    ...
    describe('header', () => {
        ...
        it('reads the first sheet and returns its name', async () => {
            const buffer = await buildWorkbook(FULL_HEADER, [ [ 'tipoEvento', 'Tipo Evento', 'ACTIVO', 'Activo' ] ], 'Catalogo');
        ...
        it('stops before reading a single data row when a required header is missing', async () => {
            ...
            expect(parsed.missingRequiredHeaders).toEqual([ 'code' ]);
        ...
        it('matches headers ignoring case, spaces, dashes and underscores', async () => {
            const buffer = await buildWorkbook(
                [ 'Catalog Type Code', 'catalog_type_name', 'CODE', 'Name', 'sort-order' ],
                [ [ 'tipoEvento', 'Tipo Evento', 'ACTIVO', 'Activo', 3 ] ]
            );
            const parsed = await parseCatalogItemsXlsxFile(buffer);
        ...
        it('reports an absent optional header and leaves its column at null, or 0 for sortOrder', async () => {
            const buffer = await buildWorkbook(
                [ 'catalogTypeCode', 'catalogTypeName', 'code', 'name', 'value' ],
        ...
        it('ignores an unknown header and reports it instead of failing', async () => {
            const buffer = await buildWorkbook(
                ...
                [ [ 'tipoEvento', 'Tipo Evento', 'ACTIVO', 'Activo', 'A', 'D', 1, 'lo que sea' ] ]
            );
            const parsed = await parseCatalogItemsXlsxFile(buffer);
    ...
    describe('normalization', () => {
        ...
        // write, and with a different rule per table. One rule for both would found a duplicate of
        // every existing type on every load
        it('applies toCamelCase to catalogTypeCode and toConstantCase to code, over the same input', async () => {
            const buffer = await buildWorkbook(FULL_HEADER, [ [ 'tipo evento', 'tipo evento', 'tipo evento', 'tipo evento' ] ]);
            const parsed = await parseCatalogItemsXlsxFile(buffer);

            expect(parsed.rows[0].catalogTypeCode).toBe('tipoEvento');
            expect(parsed.rows[0].code).toBe('TIPO_EVENTO');
        ...
        // against the type the CRUD would have created. Unifying the two code rules is another spec
        it('flattens an already-camelCase catalogTypeCode, the same way ESAVI-CATTYPE-001 does', async () => {
            const buffer = await buildWorkbook(FULL_HEADER, [ [ 'tipoEvento', 'Tipo Evento', 'ACTIVO', 'Activo' ] ]);
            const parsed = await parseCatalogItemsXlsxFile(buffer);
        ...
        it('trims value and description without normalizing them', async () => {
            const buffer = await buildWorkbook(FULL_HEADER, [
                [ 'tipoEvento', 'Tipo Evento', 'ACTIVO', 'Activo', '  valor CRUDO  ', '  Una descripción  ' ]
            ]);
            const parsed = await parseCatalogItemsXlsxFile(buffer);
    ...
    describe('row rules', () => {
        ...
        it('accepts the same code under two different types: uniqueness is of the pair', async () => {
            const buffer = await buildWorkbook(FULL_HEADER, [
                [ 'tipoEvento', 'Tipo Evento', 'ACTIVO', 'Activo' ],
                [ 'tipoVacuna', 'Tipo Vacuna', 'ACTIVO', 'Activo' ]
            ]);
            const parsed = await parseCatalogItemsXlsxFile(buffer);
        ...
        it('rejects the second appearance of the same pair: the first one wins', async () => {
            const buffer = await buildWorkbook(FULL_HEADER, [
                [ 'tipoEvento', 'Tipo Evento', 'ACTIVO', 'Activo' ],
                [ 'tipoEvento', 'Tipo Evento', 'ACTIVO', 'Activo Repetido' ]
            ]);
            const parsed = await parseCatalogItemsXlsxFile(buffer);

            expect(parsed.rows).toHaveLength(1);
            expect(parsed.rows[0].name).toBe('Activo');
        ...
        it('detects the duplicate over the normalized pair, not over the raw cells', async () => {
            const buffer = await buildWorkbook(FULL_HEADER, [
                [ 'tipo evento', 'Tipo Evento', 'activo', 'Activo' ],
                [ 'Tipo-Evento', 'Tipo Evento', 'ACTIVO', 'Activo' ]
            ]);
            const parsed = await parseCatalogItemsXlsxFile(buffer);
        ...
        it('rejects an empty catalogTypeCode, code or name, each with its own reason', async () => {
            const buffer = await buildWorkbook(FULL_HEADER, [
                [ undefined, 'Tipo Evento', 'ACTIVO', 'Activo' ],
                [ 'tipoEvento', 'Tipo Evento', undefined, 'Activo' ],
                [ 'tipoEvento', 'Tipo Evento', 'ACTIVO', undefined ]
            ]);
            const parsed = await parseCatalogItemsXlsxFile(buffer);
            ...
            expect(parsed.rejected).toEqual([
                ...
                { row: 3, reason: 'EMPTY_CODE' },
                { row: 4, reason: 'EMPTY_NAME' }
            ]);
        });

        // The parser is pure and cannot know whether the type exists, so this rejection is not its
        // business: it belongs to phase 3 of the service, and only when the type has to be created
        it('does not reject a row whose catalogTypeName is empty: it travels as null', async () => {
            const buffer = await buildWorkbook(FULL_HEADER, [
                [ 'tipoEvento', undefined, 'ACTIVO', 'Activo' ]
            ]);
            const parsed = await parseCatalogItemsXlsxFile(buffer);

            expect(parsed.rejected).toHaveLength(0);
            expect(parsed.rows[0].catalogTypeName).toBeNull();
        });

        it('rejects a text over the limit of its column, naming the column', async () => {
            const buffer = await buildWorkbook(FULL_HEADER, [
                [ 'tipoEvento', 'Tipo Evento', 'ACTIVO', 'x'.repeat(251) ],
                [ 'tipoEvento', 'Tipo Evento', 'VIGENTE', 'Vigente', 'v'.repeat(251) ]
            ]);
            const parsed = await parseCatalogItemsXlsxFile(buffer);
        ...
        it('holds catalogTypeName to 200 characters, a different limit from name', async () => {
            const buffer = await buildWorkbook(FULL_HEADER, [
                [ 'tipoEvento', 'x'.repeat(201), 'ACTIVO', 'x'.repeat(220) ]
            ]);
            const parsed = await parseCatalogItemsXlsxFile(buffer);
    ...
    describe('sortOrder coercion', () => {
        ...
        it.each([
            ...
            const buffer = await buildWorkbook(FULL_HEADER, [
                [ 'tipoEvento', 'Tipo Evento', 'ACTIVO', 'Activo', undefined, undefined, cell ]
            ]);
            const parsed = await parseCatalogItemsXlsxFile(buffer);
        ...
        it('keeps an explicit 0 as 0 without flagging it', async () => {
            const buffer = await buildWorkbook(FULL_HEADER, [
                [ 'tipoEvento', 'Tipo Evento', 'ACTIVO', 'Activo', undefined, undefined, 0 ]
            ]);
            const parsed = await parseCatalogItemsXlsxFile(buffer);
        ...
        it('keeps a valid sortOrder and the smallint ceiling itself', async () => {
            const buffer = await buildWorkbook(FULL_HEADER, [
                [ 'tipoEvento', 'Tipo Evento', 'ACTIVO', 'Activo', undefined, undefined, 7 ],
                [ 'tipoEvento', 'Tipo Evento', 'VIGENTE', 'Vigente', undefined, undefined, 32767 ]
            ]);
            const parsed = await parseCatalogItemsXlsxFile(buffer);
